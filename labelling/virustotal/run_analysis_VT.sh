...
do  
	...
	#/usr/bin/time -f "$SHA256 	Tempo decorrido da Análise do VT (%%e) = %e segundos, Tempo (%%E) = %E segundos, CPU ((%%U + %%S) / %%E) = %P, CPU nível usuário (%%U) = %U segundos, CPU nível kernel (%%S) = %S segundos" -a -o $LOG_DIR/stats-$TS.txt curl  --request GET --url "https://www.virustotal.com/api/v3/files/"$SHA256 --header  'x-apikey:'$API_KEY > $FILA_DE_LABELLING/$SHA256".json"
