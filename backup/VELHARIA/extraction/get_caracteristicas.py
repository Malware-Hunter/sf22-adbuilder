...
def get_caracteristicas(args):
    ...
    def get_op_codes(dx):
        for method in dx.get_methods():
            ...
            # Métodos externos não possuem opcodes
               continue
            idx = 0
            for ins in m.get_instructions():
                #print('{}  {}  {} '.format(idx, ins.get_op_value(), ins.get_name()))
    ...
    def get_api_calls(cg):
        ...
        with open(sha256+"_API_Calls_original.txt", "w") as file:
                            ...
                            if "java/lang/Object" not in split[0]:
                                ...
                                if comum_control == 0:
                                    if split[0] not in treatments:
                                        treatments.append(split[0])
        # criar arquivo zip
        zipfile = zp.ZipFile(sha256+"_API_Calls_original.zip", "w", zp.ZIP_LZMA)
        zipfile.write(sha256+"_API_Calls_original.txt")
        zipfile.close()
        # remover txt
        os.remove(sha256+"_API_Calls_original.txt")
    ...
    df.to_csv(str(sha256)+'.csv', index = False, encoding="utf-8-sig")
